...
async function load(path) {
  const code = ts.transpileModule(await fs.readFile(path, "utf8"), {
    compilerOptions: {
...
const cat = categories
  .map((c, i) => `(${literal(c.id)},${literal(c.name)},${i})`)
  .join(",\n");
const rows = products
  .map(
    (p) =>
      ...
      [
        ...
        .join(",") +
      "," +
      literal(JSON.stringify(p.tags)) +
      "::jsonb," +
      [p.image, p.description, p.ingredients].map(literal).join(",") +
      "," +
      literal(JSON.stringify(p.nutrition)) +
      "::jsonb," +
      literal(p.featured) +
      ")",
  )
...
const postSource = (await fs.readFile("lib/posts.ts", "utf8")).replace(
  /import .*?;\r?\n/,
  "",
);
const postCode = ts.transpileModule(postSource, {
  compilerOptions: {
    module: ts.ModuleKind.ESNext,
    target: ts.ScriptTarget.ES2022,
  },
}).outputText;
const { samplePosts } = await import(
  "data:text/javascript;base64," + Buffer.from(postCode).toString("base64")
);
const postRows = samplePosts
  .map(
    (p) =>
      "(" +
      [
...
await fs.writeFile(
  "supabase/seed.sql",
  sql +
    `insert into posts(slug,title,excerpt,content,image,published,created_at) values ${postRows} on conflict(slug) do nothing;\n`,
);
