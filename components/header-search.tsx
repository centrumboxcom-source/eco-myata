...
export default function HeaderSearch({
  ...
  return (
    <div className="live-search" ref={box}>
      <form
        className="search"
        action="/catalog"
        onSubmit={(e) => {
      ...
      >
        ...
        <input
          ...
          name="q"
          value={query}
          placeholder="Знайти щось корисне…"
          onFocus={() => {
            setOpen(true);
            void load();
          }}
      ...
      {visible && (
        <div className="search-results">
          ...
          <div role="listbox" id={id} aria-label="Знайдені товари">
            {suggestions.map((p, i) => (
              <Link
                role="option"
              ...
              >
                <Image src={p.image} alt="" width={52} height={52} />
                <span>
                  <b>{p.name}</b>
                  <small>
                    {p.weight} ·{" "}
                    {p.stock ? "Є в наявності" : "Немає в наявності"}
                  </small>
                </span>
          ...
          {!loading && !failed && !matches.length && (
            <p className="search-empty">
              Нічого не знайшли за «{query}». Спробуйте «горіхи», «чай» або
              «арахісова».
            </p>
          )}
          {failed && (
          ...
          {matches.length > 0 && (
            <Link
              ...
              onClick={close}
            >
              Показати всі товари ({matches.length}) <ArrowRight size={16} />
            </Link>
          )}
          <div className="search-hint" aria-live="polite">
            {!loading && `Знайдено: ${matches.length}`} · Результати оновлюються
            під час введення
          </div>
        </div>
      )}
